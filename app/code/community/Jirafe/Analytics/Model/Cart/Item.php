        ...
        try {
            if ($quoteId) {
                ...
                $collection = Mage::getModel('sales/quote_item')
                    ->getCollection()
                    ->getSelect()
                    ->joinLeft(array('parent'=>Mage::getSingleton('core/resource')->getTableName('sales/flat/quote/item')), "main_table.parent_item_id = parent.item_id", array('parent.row_total'))
                    ->joinLeft(array('option'=>Mage::getSingleton('core/resource')->getTableName('sales/flat/quote/item/option')), "parent.item_id = option.item_id AND option.code = 'attributes'", array('option.value'))
